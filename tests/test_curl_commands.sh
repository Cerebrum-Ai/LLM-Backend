...
echo "# ffmpeg -f lavfi -i \"sine=frequency=1000:duration=5\" tests/test_audio.wav"
echo
echo "# Then, send it to the API:"
echo "curl -X POST ${LLM_URL}/api/analyze_audio \\"
echo "  -F \"audio=@tests/test_audio.wav\""
echo
echo "# Or send directly to ML service:"
echo "curl -X POST ${ML_URL}/ml/process \\"
echo "  -H \"Content-Type: application/json\" \\"
echo "  -d '{\"url\": \"file:///path/to/tests/test_audio.wav\", \"data_type\": \"audio\", \"model\": \"emotion\"}'"
echo
echo
...
echo "curl -X POST ${ML_URL}/ml/process \\"
echo "  -H \"Content-Type: application/json\" \\"
echo "  -d '{\"url\": \"https://example.com/sample-image.jpg\", \"data_type\": \"image\", \"model\": \"diagnosis\"}'"
echo
echo
...
echo "  -H \"Content-Type: application/json\" \\"
echo "  -d '{\"question\": \"What is Parkinson's disease?\", \"context\": \"\"}'"
echo
echo
